...
it('can add the star name and star symbol properly', async() => {
    ...
    assert.equal(name, 'Pleiades star cluster');
    assert.equal(symbol, 'M45');
});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let sentName = await instance.lookUptokenIdToStarInfo.call(starId);
    // 3. Verify if you Star name is the same
    assert.equal(sentName, 'awesome star');
});